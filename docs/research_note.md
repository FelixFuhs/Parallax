...
Legacy 2026 Single-Label Distillation Result

This note documents the original Parallax research slice. It is retained as provenance for the frozen five-feature surrogate and its negative 2012-2025 historical result. The current project framing is the AI label decomposition workflow in [AI Label Decomposition v2](ai_label_decomposition_v2.md), which separates raw AI IRR, mechanical DCF IRR, AI-minus-mechanical IRR, factor-compressible score, and residual score. This legacy note should not be read as evidence of AI alpha, true historical AI-label-vintage performance, or institutional point-in-time validity.

## Abstract

Parallax originally asked whether AI-generated discounted cash flow valuations contain a cross-sectional signal that can be compressed into a small accounting model and transported backward through history. I generated GPT-5.4 Nano DCF reports for roughly 265 non-financial, non-REIT US large-cap stocks through OpenRouter, converted each base-case upside estimate into a percentile rank, and trained a surrogate on five features reconstructable from EDGAR and market data: `fcf_to_ev`, `gross_profitability_assets`, `asset_growth_1y`, `cash_earnings_gap`, and `momentum_12_1`. After freezing the specification, I rebuilt monthly accepted-date filing feature matrices from SEC filing acceptance dates and ran 2012-2025 quintile backtests with T+1 execution in both clean and broad universes. The frozen XGBoost regressor fits the present-day cross-section reasonably well, with repeated-CV Spearman around `0.62` and top-quartile stability of `90.8%`. But the historical test is negative: the long-short spread is weak, and a simple `FCF/EV` sort beats the surrogate in both universes. The contribution is methodological rather than promotional: a reusable framework for falsifying AI equity judgments with accepted-date public filing data, benchmark controls, and explicit survivor-bias caveats.
...
The surrogate is then frozen and backcast. [`historical.py`](../historical.py) reconstructs monthly accepted-date feature matrices from SEC companyfacts, with filing acceptance dates rather than fiscal period ends governing eligibility. [`backtest.py`](../backtest.py) scores each monthly cross-section with the frozen surrogate, forms quintile portfolios, and measures forward returns from the first available trading day after the rebalance date to the first available trading day after the next rebalance date.
...
The backtest is intentionally conservative about filing information timing. A historical feature vector is eligible only if the relevant annual filing was accepted by the SEC before the rebalance date. This prevents peeking at later filings and prevents pretending that fiscal year-end information was tradable before it was actually filed. It does not solve current-universe survivor bias, historical sector classification, or delisted-name coverage.
...
That is still a useful result. It suggests that the right way to evaluate AI judgment in equity research is not to admire contemporary outputs, but to force them into benchmarked, historically falsifiable tests with explicit timing and universe caveats. Parallax provides a research prototype for doing exactly that; the v2 decomposition workflow is the current implementation path.
